...
`awf dispatch --card <path-or-id> --manifest <path>` accepts the manifest and
still supports explicit legacy flags.
...
failure, and next action. `awf resume --run <id>` is fail-closed: it reports
only the one protocol-authorized next action and never replays a model or
requeues a historical delivery.
