...
1. **完整自动闭环重跑**（用户暂缓）：认证/bug 都修了，重跑证明 handler 自己 commit→push→发 `task:awf-review`
...
2. **清理旧 bash 脚本**（awf-listen.sh/roles/executors/adapters）——Python reviewer 侧验证后删，收敛配置。
3. **agent-bus stack 合并 / 开 PR**（线性 stack，未合并；合并需解 docs + ps1 冲突）。
4. **跨机 `control:shutdown` 真验**（VPS 停 Windows coder listener）——本机验过，跨机没单独验。
