...
| 项目 | 路径 | 说明 |
|---|---|---|
| agent-workflow | `/Users/torinsun/AI/01_Project/agent-workflow` | 方法本体。核心=`constitution.md` |
| agent-bus | `/Users/torinsun/AI/01_Project/agent-bus` | 第一个被开发的真实项目（VPS 事件中继） |
| ai-memory | `/Users/torinsun/AI/ai-memory` | 跨 agent 长期记忆，**优先读** |
...
当前：Claude(architect) 经 **VPS Agent Bus** 发事件 → 本机 coder listener → 触发本机 OpenCode 执行。
- VPS：`ssh tx-vps`；Agent Bus `http://100.108.67.47:8800`，per-agent token（architect/coder，在
  VPS `/etc/agent-bus/.env`，读取需 root；本地 architect token 在 agent-bus `.env`）。
- 本机 OpenCode：`opencode run [-m opencode-go/deepseek-v4-flash] "<msg>"`，workdir 用 handler `cd`。
