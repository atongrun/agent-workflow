...
def architect_context(
    *,
    ...
    return (
        ...
        "literal HTML comment, including its markers and strict JSON double quotes:\n"
        "<!-- awf-reviewer-selection\n"
        f"{selection_json}\n"
        "-->\n"
        "Do not render that contract as a heading, code fence, bare JSON, or prose. The required "
        "awf-postflight contract is likewise one literal HTML comment with strict JSON.\n\n"
        "## Durable facts\n\n```json\n"
...
def next_architect_context(
    *,
    ...
    return (
        ...
        f"suffix: - **Frozen base**: `{fresh_main}`. Include exactly this literal HTML comment:\n"
        "<!-- awf-reviewer-selection\n"
        f"{selection_json}\n"
        "-->\n"
        "Do not render that contract as a heading, code fence, bare JSON, or prose. "
        "The awf-postflight allowed_paths must contain exactly one "
        "`.awf/artifacts/impl-report-<TASK_ID>.md` and one "
        "`.awf/artifacts/review-report-<TASK_ID>.md`; the TaskCard path itself must not be "
        "allowed.\n\n"
        "## Durable facts\n\n```json\n"
        + json.dumps(facts, ensure_ascii=False, indent=2, sort_keys=True)
