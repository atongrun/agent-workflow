...
def _validate_profile_semantics(profile: NodeProfile) -> None:
    ...
    if profile.role == "coder" and profile.values["tool"] not in {"opencode", "codex", "pi"}:
        raise NodeError("coder tool must be OpenCode, Codex, or Pi")
    if profile.role == "architect" and profile.values["tool"] not in {
        ...
        raise NodeError(
            "architect tool must be Pi, OpenCode, Codex, or the internal no-model terminal consumer"
        )
