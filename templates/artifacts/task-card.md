...
<!--
A TaskCard is a SELF-CONTAINED handoff package. An Executor (an execution model or human,
possibly on another machine, in a fresh session with no chat history) must be
...
- **Repository**: [repository identifier; use a repo-relative checkout placeholder, not a personal path]
...
- **Explicit inputs**: [all files/artifacts/environment-variable names required to start]
...
- **Escalate (stop and report)** if: the goal is ambiguous, required context is missing,
  frozen architecture must reopen, a predefined high-risk boundary is entered, evidence cannot
  support a reliable decision, or bounded rework is exhausted. Ordinary failures above do not
  escalate merely because the first attempt failed.
...
- [ ] The Executor does not need to search AI Memory for a fact required to complete the task.
